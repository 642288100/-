...
def simpson_rule(f, a, b, n):
    ...
    x = np.linspace(a, b, n+1)
    y = f(x)
    integral = (h / 3) * (y[0] + 4 * np.sum(y[1::2]) + 2 * np.sum(y[2:n:2]) + y[-1])
    return integral

integral_approximation_simpson = simpson_rule(lambda t: np.interp(t, t_values, u_values), t0, T, num_steps)

# 使用Gauss积分公式计算积分：
def gauss_quadrature(f, a, b, n):
    x, w = np.polynomial.legendre.leggauss(n)
    integral = ((b - a) / 2) * np.sum(w * f(0.5 * (b - a) * x + 0.5 * (b + a)))
    return integral

integral_approximation_gauss = gauss_quadrature(lambda t: np.interp(t, t_values, u_values), t0, T, num_steps)

#输出结果：
print("Approximation using Runge-Kutta method:", u_values[-1])
print("Approximation using Trapezoidal Rule:", integral_approximation)
print("Approximation using Simpson Rule:", integral_approximation_simpson)
print("Approximation using Gauss Quadrature:", integral_approximation_gauss)
