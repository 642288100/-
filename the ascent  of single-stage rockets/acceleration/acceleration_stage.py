...
def runge_kutta_4_stage3(f, t_start, v_start, dt, steps):
    t_values = [t_start]
    v_values = [v_start]
    for _ in range(steps):
        k1 = f(None, v_start)  # 第二阶段速度函数不依赖于时间
...
v_values_3, t_values_3 = runge_kutta_4_stage3(velocity_prime_stage3, t2, v_t2, (t3 - t2) / h, h)
...
# 计算第三阶段加速度数据
acceleration_values_3 = [velocity_prime_stage3(None, v) for v in v_values_3]
...
stage_3,=plt.plot(t_values_3, acceleration_values_3,label="acceleration_stage3",color="green")
plt.xlabel("time (s)")
plt.ylabel("acceleration (m/s^2)")
plt.title("Acceleration_stage VS Time (Runge-Kutta method)")
plt.grid(True)
plt.legend(handles=[stage_1, stage_2,stage_3], loc='lower right')
plt.show()

# 保存绘图
plt.savefig("acceleration_stage_all.png", dpi=500)
plt.close()